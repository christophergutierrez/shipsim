...
#[test]
fn scenarios_without_a_map_mode_default_to_unbounded() {
    let game = load("scenarios/ai.toml");
    assert_eq!(game.board().mode, MapMode::Unbounded);
}
...
fn unbounded_mode_never_blocks_edges() {
    ...
    assert_eq!(ship1.facing, 0, "ship 1 should now face 0 (+q)");
...
fn hard_mode_still_blocks_edge_exits() {
    // The bounded combat scenario explicitly requests Hard mode. A ship trying
    // to exit the board must still be blocked.
